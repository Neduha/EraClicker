...
class GameViewModel : ViewModel() {
    ...
    var resources by mutableStateOf(1000)
        private set
    ...
    var currentEra by mutableStateOf(1)
        private set
    ...
    fun buyUpgrade(id: Int) {
        val index = upgrades.indexOfFirst { it.id == id && it.era == currentEra }
        if (index < 0) return
        val up = upgrades[index]
        if (resources < up.cost) return
